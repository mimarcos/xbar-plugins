...
# <xbar.title>TOTP Authenticator</xbar.title>
# <xbar.version>v1.0</xbar.version>
# <xbar.author>Michael Marcos</xbar.author>
# <xbar.author.github>mimarcos</xbar.author.github>
# <xbar.desc>Use this to generate and copy TOTP tokens.</xbar.desc>

export LANG="${LANG:-en_US.UTF-8}"

# Installation steps

# 1) install oathtoolkit (brew install oath-toolkit) 
# 2) update the path for oathtool below
# 3) add secrets to totp_secrets (label:secret)
# 4) add as xbar plugin, 001-totp.5m.sh
# 5) profit

 totp_secrets=( "Label:SECRET" 
   "Label2:SECRET2" )

oathtool="/opt/homebrew/bin/oathtool"

# copy code if argument passed in
if [[ "$1" == "copy" ]]; then
  token=$( $oathtool --totp -b "$2" )
  echo -n "$(echo -n "$token")" | pbcopy
  exit
fi

# clear clipboard if desired
if [[ "$1" == "clear" ]]; then
  echo -n "" | pbcopy
  exit
fi
...
echo '---'
echo "Clear Clipboard | bash='$0' param1=clear terminal=false"
echo "---"

for secret in "${totp_secrets[@]}" ; do
  KEY="${secret%%:*}"
  SECRET="${secret##*:}"
  echo "$KEY | bash=$0 param1=copy param2='$SECRET' terminal=false"
done
